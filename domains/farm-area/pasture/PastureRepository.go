...
func NewRepository(db *sqlx.DB, userId string) *PastureRepository {
	selectQuery := `
    ...
    `
	return &PastureRepository{selectQuery, "pastures", db, userId}
}

func (r *PastureRepository) FindAll() (*[]Pasture, error) {
	query := r.SelectQuery + " WHERE pastures.user_id = $1 AND pastures.deleted_at is null"
	return repositoriesUtil.GetList[Pasture](r.Db, query, r.UserId)
}
