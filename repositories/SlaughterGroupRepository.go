...
func (r *SlaughterGroupRepository) Init() {
	selectQuery := util.NewSelectQuery(util.SELECT,
		*util.NewNamedGroup("group", "id", "weight_decrease", "slaughter_date"),
		*util.NewNamedGroup("slaughterhouses", "id", "name")).
		From("slaughter_groups as group").
		Joins("slaughterhouses on slaughterhouses.id = group.slaughterhouse_id")

	updateQuery := util.NewUpdateQuery("slaughter_groups", "weight_decrease", "slaughter_date",
		"slaughterhouse_id", "created_at", "user_id")
	insertQuery := util.NewInsertQuery("slaughter_groups", "id", "weight_decrease", "slaughter_date",
		"slaughterhouse_id", "created_at", "user_id")
